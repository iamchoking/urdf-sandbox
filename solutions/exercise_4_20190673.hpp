...
public:
  ...
  void calcLinkNE(Eigen::VectorXd extF = Eigen::VectorXd::Zero(6)){ //reset with acc (often uses same condition)
    ...
    // fi = getSpatialInertia() * fullA + getSpatialBiasVector() - extF; // the famous NE!
    fi = getSpatialInertia() * fullA + getSpatialBiasVector() + extF; // the famous NE!
  ...
  Eigen::VectorXd getSpatialBiasVector(){
    Eigen::VectorXd bi;
    bi.resize(6);
...
public:
  ...
  void calculateNonlinearTermRNE(bool gravity = true){
    ...
    worldAcc << 0,0,9.81*gravity,0,0,0;
    gvdot.resize(gvDim);
    gvdot.setZero();
