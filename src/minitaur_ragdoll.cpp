...
int main(int argc, char* argv[]) {
  ...
  // minitaur -> setComputeInverseDynamics(true);
  ...
  gcDim_ = minitaur->getGeneralizedCoordinateDim();
  gvDim_ = minitaur->getDOF();
  ...
  gc.setZero();
  gc[5] = 1.0; // required (quaternion) for floating base gc init
  gc[2] = 1.0;
  gv.setZero();
  ...
  minitaur->setGeneralizedForce(Eigen::VectorXd::Zero(gvDim_));

  // utils::gcRandomize(gc);
  // gc[2] = gc[2] + 3;
  // utils::gvRandomize(gv,15);

  minitaur->setState(gc, gv);
  server.launchServer();
  ...
  for (size_t t = 0; t<TOTAL_STEPS; t++){
    ...
    minitaur->getState(gc, gv);
    
    // analyze step here
    std::cout<<"STEP " << t << "/" << TOTAL_STEPS << std::endl;

    
    // z = minitaur->getBodyCOM_W()[2][2];
    // std::cout << "  CoM Height: " << z*1000 << "mm" << std::endl;
    // minitaur->getFramePosition("chin_d455",pos);
    // std::cout << "  d455 position  : " << pos.e().transpose() << std::endl;
    // minitaur->getFramePosition("chin_mid360",pos);
    ...
    // minitaur->setPdTarget(pTarget_,dTarget_);
