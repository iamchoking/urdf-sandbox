...
public:
  ...
  bool ik(
    ...
    int multistepIter = 0;
    const int maxMultisteps = 50;
    bool finalConverged = false;
    
    while (multistepIter < maxMultisteps) {
      ...
      for (int iter = 0; iter < cfg.maxIterations; ++iter) {
        ...
        // Compute Jacobians
        Eigen::MatrixXd J_pos9(3,9), J_rot9(3,9), J_7(6,7);
        J_pos9.setZero();
        J_rot9.setZero();
        J_7.setZero();
        ...
        const Eigen::MatrixXd Nproj = Eigen::MatrixXd::Identity(7,7) - Jplus * J_7;
        ...
        while (!stepAccepted) {
          ...
          if((candidateGc - sol).cwiseAbs().maxCoeff() <= cfg.maxJointDistance &&
             isImprovement(candidateGc, stepTargetPos, stepTargetRot, dPos, dRot, frameName, cfg)) {
            stepAccepted = true;
            sol = gc_;
